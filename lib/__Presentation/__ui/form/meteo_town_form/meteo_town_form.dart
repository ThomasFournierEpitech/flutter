...
import 'package:app_meteo/__Presentation/__ui/page/meteo_detail_page.dart';
import 'package:app_meteo/__Presentation/bloc/component/meteo_detail_bloc/meteo_detail_cubit.dart';
import 'package:app_meteo/__Presentation/bloc/form/meteo_town_form/meteo_town_form_cubit.dart';
    ...
    return MultiBlocListener(
      listeners: [
        ...
        BlocListener<MeteodetailCubit, MeteoInfoState>(
          ...
          if (state.status != MeteoInfoStateStatus.error && state.status != MeteoInfoStateStatus.unknow) {
            Navigator.push(
              context,
              MaterialPageRoute(
          ...
          } else if (state.status == MeteoInfoStateStatus.error) {
            ScaffoldMessenger.of(context)
              ..hideCurrentSnackBar()
              ..showSnackBar(
                SnackBar(
                  content: Text("${state.resolvedAddress} not found"),
                ),
              );
          }
        }),
